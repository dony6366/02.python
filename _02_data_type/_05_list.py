...
#list 저장 요소 추가/삭제
#- list는 동적으로 크기 변경이 가능한 mutable 자료형이가
...
print("append 전 후 같은 list인가?",before_id ==id(lst) )

# list.insert(index, 값)
# - index에 값을 삽입하는 메서드
# - 지정된 index 부터 뒤에있는 모든 list 값의 index가 1씩 증가(밀려남)
print("--- list.insert()")
print(lst.insert(1,1.5))
...
print(len(students[0])) # 2(여ㅑㄹ)
print(len(students[0][0])) # 3 (홍길동 글자 수)

#str.split(구분자)
# - str을 구분자를 기준으로 나워서 list 형태로 반환
data = "홍길동,20,서울시,서초구" # CSV(Comma Separated Value)- 콤마로만 구분되는 문장
data_ = data.split(',')
